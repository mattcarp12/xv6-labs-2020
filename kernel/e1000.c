...
int e1000_transmit(struct mbuf *m)
{
  ...
  struct tx_desc *txd = &tx_ring[tx_ring_index];

  // Check if device is ready to transmit next packet
  if (!txd_stat_dd(txd))
    goto error;

  // Free last mbuf that was transmitted from descriptor
  struct mbuf *txm = tx_mbufs[tx_ring_index];
  if (txm)
    mbuffree(txm);
